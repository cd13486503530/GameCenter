...
(function ($) {
    ...
    var isIE, isEdge, handler, previewCache, getNum, hasFileAPISupport, hasDragDropSupport, hasFileUploadSupport, addCss,
        ...
        tZoom, tGeneric, tHtml, tImage, tText, tVideo, tAudio, tFlash, tObject, tOther, tXlPreview, defaultLayoutTemplates,
    ...
    '</div>';
    tXlPreview = '<div class="file-preview-frame{frameClass}" id="{previewId}">' +
        '<span class="glyphicon glyphicon-facetime-video"></span>&nbsp;' + 
        '<strong class="xlpreview" style="cursor: pointer" data-qid="{QuestionID}" data-sid="{ID}" data-filekey="{FileKey}" data-size="{FileSize}" data-ext="{Extension}">{FileKey}.{Extension}</strong>' + 
        '</div>';
    ...
    defaultFileTypeSettings = {
        ...
        video: function (vType, vName) {
            return (vType !== undefined && vType.match(/\.video\/(ogg|mp4|webm|3gp|avi)$/i)) || vName.match(/\.(og?|mp4|webm|3gp|avi)$/i);
        },
    ...
    FileInput.prototype = {
        ...
        init: function (options) {
            ...
            self.autoUpload = options.uploadRightNow;
        ...
        renderFileFooter: function (caption, width) {
            ...
            if (self.isUploadable) {
                footer = template.replace(/\{actions}/g, self.renderFileActions(self.showUpload, self.showRemove, false, false, false));
        ...
        previewFile: function (i, file, theFile, previewId, data) {
            ...
            var self = this, cat = self.parseFileType(file), fname = file ? file.name : '', caption = self.slug(fname),
                ...
                tmplt = self.getPreviewTemplate(self.defaultPreviewTypes ? self.defaultPreviewTypes : cat),
                chkTypes = types && types.indexOf(cat) >= 0, id,
